...
为避免出现“当前激活虚拟环境”和“项目 `.venv`”不一致导致的依赖混用问题，项目中的 shell 脚本已经统一改成通过 `uv run --active python ...` 调用 Python 主程序。
...
如果你更喜欢直接运行主程序，也可以手动执行：

```bash
uv run --active python -m src.train.train_grpo --config configs/grpo_qwen25_1_5b.yaml
```
...
```bash
uv run --active python scripts/download_model.py --config configs/grpo_qwen25_1_5b.yaml
```

3. 下载并处理数据集

```bash
uv run --active python scripts/download_dataset.py --config configs/grpo_qwen25_1_5b.yaml
```
...
```bash
uv run --active python src/eval/summarize_results.py --eval_dir ./outputs/eval
```
...
SwanLab 一般不需要你先在网页手动创建项目。训练开始后，`swanlab.init(...)` 会按配置中的 `project.name` 自动在你的空间下创建项目和实验记录。
